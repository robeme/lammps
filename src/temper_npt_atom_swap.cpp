...
#include "temper_npt_atom_swap.h"
#include <cmath>
#include <cstring>
#include "universe.h"
#include "domain.h"
...
TemperNPTAtomSwap::TemperNPTAtomSwap(LAMMPS *lmp) : Pointers(lmp) {}

/* ---------------------------------------------------------------------- */

TemperNPTAtomSwap::~TemperNPTAtomSwap()
{
  MPI_Comm_free(&roots);
...
void TemperNPTAtomSwap::command(int narg, char **arg)
{
  ...
  double press_set = force->numeric(FLERR,arg[7]);

  // ignore temper command, if walltime limit was already reached

  if (timer->is_timeout()) return;

  for (nptfix = 0; nptfix < modify->nfix; nptfix++)
    if (strcmp(arg[3],modify->fix[nptfix]->id) == 0) break;
  if (nptfix == modify->nfix)
    error->universe_all(FLERR,"Tempering fix ID is not defined");

  for (swpfix = 0; swpfix < modify->nfix; swpfix++)
    if (strcmp(arg[4],modify->fix[swpfix]->id) == 0) break;
  if (swpfix == modify->nfix)
    error->universe_all(FLERR,"atom/swap fix ID is not defined");

  seed_swap = force->inumeric(FLERR,arg[5]);
  seed_boltz = force->inumeric(FLERR,arg[6]);

  my_set_temp = universe->iworld;
  if (narg == 9) my_set_temp = force->inumeric(FLERR,arg[8]); // replica when restarting temper run
  ...
  if ( (!utils::strmatch(modify->fix[nptfix]->style,"^npt")) &&
  ...
  if (narg == 9) {
    ...
    modify->fix[nptfix]->reset_target(new_temp);
    modify->fix[swpfix]->reset_target(new_temp);
  }
...
void TemperNPTAtomSwap::scale_velocities(int t_partner, int t_me)
{
...
void TemperNPTAtomSwap::print_status()
{
  if (universe->uscreen) {
