...
FixConpWire::FixConpWire(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if (narg < 11) error->all(FLERR,"Illegal fix conp/wire command");
  maxiter = 100;
  tolerance = 0.000001;
...
void FixConpWire::setup(int vflag)
{
  g_ewald = force->kspace->g_ewald;
  wire_volfactor = force->kspace->wire_volfactor;
...
//copied from ewald.cpp end
  ...
  memory->create3d_offset(cs,-kmax,kmax,3,nmax,"fixconpwire:cs");
  memory->create3d_offset(sn,-kmax,kmax,3,nmax,"fixconpwire:sn");
...
void FixConpWire::pre_force(int vflag)
{
...
int FixConpWire::electrode_check(int atomid)
{
...
/*----------------------------------------------------------------------- */
void FixConpWire::equation_solve()
{
//solve equations
  if (minimizer == 0) {
    cgtime1 = MPI_Wtime();
...
void FixConpWire::a_cal()
{
  ...
  double **eleallx = NULL;
  memory->create(eleallx,elenum_all,3,"fixconpwire:eleallx");
  ...
  memory->create(csk,kcount,elenum_all,"fixconpwire:csk");
  memory->create(snk,kcount,elenum_all,"fixconpwire:snk");
  sincos_a(eleallx);
  delete [] elexyzlist;
...
void FixConpWire::sincos_a(double **eleallx)
{
  int i,m,k,ic;
  int kx,ky,kz;
  double ***csele,***snele;
  memory->create3d_offset(csele,-kmax,kmax,3,elenum_all,"fixconpwire:csele");
  memory->create3d_offset(snele,-kmax,kmax,3,elenum_all,"fixconpwire:snele");
...
/* ---------------------------------------------------------------------- */
void FixConpWire::force_cal(int vflag)
{
  int i;
...
/* ---------------------------------------------------------------------- */
void FixConpWire::coul_cal(int coulcalflag,double *m,int *ele2tag)
{
  Ctime1 = MPI_Wtime();
...
/* ---------------------------------------------------------------------- */
double FixConpWire::rms(int km, double prd, bigint natoms, double q2)
{
