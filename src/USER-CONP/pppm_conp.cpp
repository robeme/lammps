...
PPPMConp::PPPMConp(LAMMPS *lmp) : KSpace(lmp),
...
void PPPMConp::init()
{
  ...
  if (domain->triclinic && slabflag)
    error->all(FLERR,"Cannot (yet) use PPPM/conp with triclinic box and "
               "slab correction");
  ...
  if (order < 2 || order > MAXORDER)
    error->all(FLERR,fmt::format("PPPM/conp order cannot be < 2 or > {}",MAXORDER));
  ...
  while (order >= minorder) {
    if (iteration && me == 0)
      error->warning(FLERR,"Reducing PPPM/conp order b/c stencil extends "
  ...
  if (order < minorder) error->all(FLERR,"PPPM/conp order < minimum allowed order");
  if (!overlap_allowed && !gctmp->ghost_adjacent())
    error->all(FLERR,"PPPM/conp grid stencil extends "
               "beyond nearest neighbor processor");
...
void PPPMConp::allocate()
{
  ...
  memory->create2d_offset(rho_coeff,order,(1-order)/2,order/2,"pppm/conp:rho_coeff");
...
void PPPMConp::set_grid_local()
{
  ...
  if (comm->layout != Comm::LAYOUT_TILED) {
    ...
    nyhi_in = static_cast<int> 
      (comm->ysplit[comm->myloc[1]+1] * ny_pppm/wire_volfactor) - 1;
...
void PPPMConp::compute_group_group(int groupbit_A, int groupbit_B, int AA_flag)
{
  if ((wireflag || slabflag) && triclinic)
    error->all(FLERR,"Cannot (yet) use K-space slab or wire "
  ...
  if (wireflag == 1)
    wirecorr_groups(groupbit_A, groupbit_B, AA_flag);
}
...
void PPPMConp::allocate_groups()
{
  ...
  memory->create3d_offset(density_A_brick,nzlo_out,nzhi_out,nylo_out,nyhi_out,
                          nxlo_out,nxhi_out,"pppm/conp:density_A_brick");
  memory->create3d_offset(density_B_brick,nzlo_out,nzhi_out,nylo_out,nyhi_out,
                          nxlo_out,nxhi_out,"pppm/conp:density_B_brick");
