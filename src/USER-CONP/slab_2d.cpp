...
void Slab2d::compute_corr(double /*qsum*/, int eflag_atom, int eflag_global,
  ...
  MPI_Allgatherv(q, nlocal, MPI_DOUBLE, &q_all.front(), &recvcounts.front(),
                 &displs.front(), MPI_DOUBLE, world);
  MPI_Allgatherv(&z.front(), nlocal, MPI_DOUBLE, &z_all.front(),
...
void Slab2d::vector_corr(bigint *imat, double *vec) {
  ...
  int n_electrolyte;
  MPI_Allreduce(&n_electrolyte_local, &n_electrolyte, 1, MPI_INT, MPI_SUM,
                world);
  ...
  vector<int> displs = gather_displs(recvcounts);
  MPI_Allgatherv(&z_local.front(), n_electrolyte_local, MPI_DOUBLE,
                 &z_all.front(), &recvcounts.front(), &displs.front(),
                 MPI_DOUBLE, world);
  MPI_Allgatherv(&q_local.front(), n_electrolyte_local, MPI_DOUBLE,
                 &q_all.front(), &recvcounts.front(), &displs.front(),
                 MPI_DOUBLE, world);
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    for (size_t j = 0; j < z_all.size(); j++) {
      ...
      b +=
          q_all[j] * (exp(-(gzij * gzij)) / g_ewald + MY_PIS * zij * erf(gzij));
    }
    vec[ii] -= prefac * b;
  }
}
...
void Slab2d::matrix_corr(bigint *imat, double **matrix) {
  ...
  MPI_Allgatherv(&nprd_local.front(), ngrouplocal, MPI_DOUBLE,
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    for (bigint j = 0; j < ngroup; j++) {
      ...
      double aij = prefac * (exp(-dij * dij * g_ewald_sq) * g_ewald_inv +
