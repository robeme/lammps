  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "pair") == 0) {
      ...
      if (strcmp(arg[iarg + 1], "off") == 0)
        molflag = OFF;
      else if (strcmp(arg[iarg + 1], "inter") == 0)
        molflag = INTER;
      else if (strcmp(arg[iarg + 1], "intra") == 0)
        molflag = INTRA;
      else
...
void ComputeGroupGroup::init()
{
  ...
  if (kspaceflag) {
    ...
    if ((fabs(e_correction) > SMALL) && (comm->me == 0))
      error->warning(FLERR,
                     "Both groups in compute group/group have a net charge; "
  ...
  if (pairflag) neighbor->add_request(this, NeighConst::REQ_OCCASIONAL);
}
...
void ComputeGroupGroup::kspace_correction()
{
  ...
  for (int i = 0; i < atom->nlocal; i++) {
    if ((mask[i] & groupbit_A) && (mask[i] & groupbit_B)) qsqsum_group += q[i] * q[i];
