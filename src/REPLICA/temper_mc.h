...
#ifdef COMMAND_CLASS
// clang-format off
CommandStyle(temper/mc,TemperMC);
// clang-format on
#else

#ifndef LMP_TEMPERMC_H
#define LMP_TEMPERMC_H

#include "command.h"

namespace LAMMPS_NS {

class TemperMC : public Command {
 public:
  TemperMC(class LAMMPS *);
  ~TemperMC() override;
 ...
 private:
  ...
  class Fix *whichfixt;                 // temperature fix to use
  class Fix *whichfixmc;                // mc fix to use
