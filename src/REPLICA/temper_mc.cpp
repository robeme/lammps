...
TemperMC::TemperMC(LAMMPS *lmp) : Command(lmp) {}
...
void TemperMC::command(int narg, char **arg)
{
  if (universe->nworlds == 1)
    error->universe_all(FLERR,"More than one processor partition required for temper command");
  if (domain->box_exist == 0)
    error->universe_all(FLERR,"Temper/mc command before simulation box is defined");
  if (narg != 7 && narg != 8) error->universe_all(FLERR,"Illegal temper command");
  ...
  whichfixt = modify->get_fix_by_id(arg[3]);
  if (!whichfixt)
  ...
  if ((!utils::strmatch(whichfixt->style,"^nvt")) &&
      (!utils::strmatch(whichfixt->style,"^langevin")) &&
      (!utils::strmatch(whichfixt->style,"^gl[de]$")) &&
      (!utils::strmatch(whichfixt->style,"^rigid/nvt")) &&
      (!utils::strmatch(whichfixt->style,"^temp/")))
    error->universe_all(FLERR,"Tempering temperature fix is not supported");
  
  whichfixmc = modify->get_fix_by_id(arg[4]);
  if (!whichfixmc)
    error->universe_all(FLERR,fmt::format("Tempering fix ID {} is not defined", arg[4]));
    
  if ((!utils::strmatch(whichfixmc->style,"^atom/swap")))
    error->universe_all(FLERR,"Swap fix is not supported");
  ...
  if (narg == 8) {
    ...
    whichfixt->reset_target(new_temp);
    whichfixmc->reset_target(new_temp);
  }
...
void TemperMC::scale_velocities(int t_partner, int t_me)
{
